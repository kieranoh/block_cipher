...
void decrypt_pixel_data_cfb(BMPData *bmpData, const uint8_t *key, const uint8_t *iv) {
    ...
    AES_KEY aes_key;
    if (AES_set_encrypt_key(key, 128, &aes_key) < 0) {  // Same encryption key used for decryption
    ...
    for (size_t i = 0; i < bmpData->pixelDataSize; i += 16) {
        ...
        // Encrypt feedback using AES (same as encryption process)
        AES_encrypt(feedback, encrypted_block, &aes_key);

        // XOR encrypted feedback with ciphertext to recover plaintext
        for (size_t j = 0; j < block_size; j++) {
            bmpData->pixelData[i + j] = block[j] ^ encrypted_block[j];
        }

        // Update feedback with the ciphertext block (CFB chaining)
        memcpy(feedback, block, 16);
    }

    printf("Finished CFB decryption.\n");
}

int main() {
    const char *encrypted_bmp_file = "cfb_result/output.bmp";
    const char *output_bmp = "cfb_result/decrypted_output.bmp";
    ...
    decrypt_pixel_data_cfb(bmpData, key, iv);
