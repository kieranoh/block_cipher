...
void encrypt_pixel_data_ofb(BMPData *bmpData, const uint8_t *key, const uint8_t *iv) {
    ...
    for (size_t i = 0; i < bmpData->pixelDataSize; i += 16) {
        ...
        // XOR encrypted feedback (key stream) with plaintext to produce ciphertext
    ...
    printf("Finished OFB encryption.\n");
}

int main() {
    const char *bmp_file = "input.bmp";
    const char *output_bmp = "ofb_result/output.bmp";
    ...
    encrypt_pixel_data_ofb(bmpData, key, iv);
