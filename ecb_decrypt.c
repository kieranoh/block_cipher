...
void decrypt_pixel_data_ecb(uint8_t *data, size_t data_size, const uint8_t *key, const char *output_file, const char *input_file) {
    AES_KEY aes_key;
    ...
    for (size_t j = 0; j < 16; j++) {
        fprintf(output, "%02x", decrypted_block[j]); 
    }
    fprintf(output, "\n"); 
}
...
int main() {
    const char *encrypted_bmp = "ecb_result/output.bmp";
    const char *txt_file = "ecb_result/temp.txt";
    const char *decrypted_txt = "ecb_result/decrypted.txt";
    const char *decrypted_bmp = "ecb_result/decrypted.bmp";
    uint8_t key[16] = "1234567890abcde"; 

    // transform BMP as txt
    save_bmp_to_txt(encrypted_bmp, txt_file);

    size_t data_size;
    uint8_t *pixel_data = read_pixel_data_from_txt(txt_file,&data_size);

    decrypt_pixel_data_ecb(pixel_data,data_size,key,decrypted_txt,txt_file);

    // transform txt as BMP
    load_txt_to_bmp(decrypted_txt, decrypted_bmp);
